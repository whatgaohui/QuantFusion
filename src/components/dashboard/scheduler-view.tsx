...
export function SchedulerView() {
  ...
  const schedulerStatusColor = (status: string) => {
    switch (status) {
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
              <div className="p-3 bg-[#0a0a0f] rounded-lg border border-[#1e1e2e]">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span className={`text-sm font-medium text-${schedulerStatusColor(scheduler.status)}-400`}>
      ...
      <Card className="bg-[#111118] border-[#1e1e2e] rounded-xl">
        <CardHeader className="pb-3">
          <CardTitle className="text-sm font-medium text-zinc-300">
            {language === 'zh' ? `任务列表 (${jobs.length})` : `Tasks (${jobs.length})`}
          </CardTitle>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <div className="text-center py-8 text-zinc-500 text-sm">{t('sched.noJobs')}</div>
          ) : (
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 text-[11px] text-zinc-500">
                        ...
                        <span>•</span>
                        <span>{t('sched.lastRun')}: <span className={statusColor(job.lastStatus)}>{formatTime(job.lastRunAt)}</span></span>
                        <span>•</span>
